...
if len(sys.argv) >3:
	# Usage: python simple2lemon_many_files.py [file.csv] [namespace] 
	...
	print "Usage: python simple2lemon_many_files.py [file.csv] [psc] [singlepsc] [mapping]"
	sys.exit()
...
for lemma, sense_list in lexicon.iteritems():
	...
	file_uniq.write ( d_tab+"<lemon:LexicalEntry rdf:ID=\""+ lemma + "\">\n" )
	...
	for sense in sense_list: 
		...
		if myrels is not None:
			for rels in lemonrelations.get( sense ):
				rel = rels.split("#")[0]
